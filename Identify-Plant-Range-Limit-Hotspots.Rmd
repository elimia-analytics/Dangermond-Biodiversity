...
# Load plant species distribution models from Kling et al. (2018)
...
dir.create("range_limit_rasters")
model_files <- list.files("data/Kling_et_al_2018", full.names = TRUE)
future::plan("multisession", workers = 3)
furrr::future_map(model_files[4400:length(model_files)], function(mod){
  prob_raster <- terra::rast(mod)
  r <- prob_raster
  values(r)[which(values(r) == 0)] <- NA
  r <- r %>% terra::trim()
  if (terra::ymax(r) <= -72000){
    n_edge_raster <- calculate_range_limit_probabilities(probability_raster = prob_raster, limit = "northern")
    terra::writeRaster(n_edge_raster$latitudinal_probability_raster, paste0("range_limit_rasters/", prob_raster %>% names(), "_northern_probability.tif"), overwrite = TRUE)
    terra::writeRaster(n_edge_raster$edge_raster, paste0("range_limit_rasters/", prob_raster %>% names(), "_northern_edge_limit.tif"), overwrite = TRUE)
  }
  if (terra::ymin(r) >= -590000){
    s_edge_raster <- calculate_range_limit_probabilities(probability_raster = prob_raster, limit = "southern")
    terra::writeRaster(s_edge_raster$latitudinal_probability_raster, paste0("range_limit_rasters/", prob_raster %>% names(), "_southern_probability.tif"), overwrite = TRUE)    
    terra::writeRaster(s_edge_raster$edge_raster, paste0("range_limit_rasters/", prob_raster %>% names(), "_southern_edge_limit.tif"), overwrite = TRUE)
  }
