...
gbif_occurrences_sb_clean <- gbif_occurrences_sb %>% 
  ...
  dplyr::mutate(scientificName = scientificname, longitude = decimallongitude, latitude = decimallatitude, source = "GBIF", dataset = institutioncode, key = gbifid, coordinateUncertaintyInMeters = coordinateuncertaintyinmeters, eventDate = eventdate, URL = paste0("https://www.gbif.org/occurrence/", gbifid), year = year) %>% 
  dplyr::select(data_fields) %>% 
  dplyr::mutate(key = as.character(key), eventDate = as.character(eventDate), year = as.numeric(year), longitude = as.numeric(longitude), latitude = as.numeric(latitude))
...
integrated_dangermond_occurrences <- list(calflora_occurrences_dangermond_clean,
  ...
  bind_rows()
# Save output as CSV
integrated_dangermond_occurrences %>% readr::write_csv(paste0("data/integrated_occurrences_dangermond.csv"))
```
...
dangermond_taxa_info_gnr <- furrr::future_map(dangermond_taxa, purrr::safely(function(x) taxize::gnr_resolve(x, fields = "all", best_match_only = TRUE, canonical = TRUE, preferred_data_sources = c(11))), seed = TRUE)
## Extract taxonomic information from GBIF Backbone Taxonomy
dangermond_taxa_info_gbif <- purrr::map(dangermond_taxa_info_gnr, "result") %>% 
  bind_rows() %>% 
