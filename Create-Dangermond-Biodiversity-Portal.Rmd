...
gbif_occurrences_dangermond <- spocc::occ(from = "gbif", limit = gbif_occurrence_count_dangermond, geometry = dangermond_preserve_bbox, date = c("1980-01-01", Sys.Date()), has_coords = TRUE, )$gbif$data[[1]]
### Save output as csv
...
gbif_occurrences_sb <- gbif_occurrences_california %>% 
  dplyr::filter(decimallongitude >= -120.6723 & decimallongitude <= -119.0274 & decimallatitude >= 33.46569 & decimallatitude <= 35.11459, year >= 1980)
...
### Save output as csv
gbif_occurrences_sb %>% sf::st_set_geometry(NULL) %>% readr::write_csv("data/gbif_occurrences_sb.csv")
```
...
## GBIF
gbif_occurrences_dangermond_clean <- gbif_occurrences_dangermond %>% 
  dplyr::filter(institutionCode != "iNaturalist") %>% # Remove iNaturalist records because those are being extracted directly from iNaturalist
  dplyr::mutate(source = "GBIF", dataset = institutionCode, key = key, eventDate = eventDate, URL = paste0("https://www.gbif.org/occurrence/", key), year = year) %>% 
  dplyr::select(data_fields) %>% 
  dplyr::mutate(key = as.character(key), eventDate = as.character(eventDate), year = as.numeric(year), longitude = as.numeric(longitude), latitude = as.numeric(latitude))

## iNaturalist (General Search)
inat_occurrences_dangermond_clean <- inat_occurrences_dangermond %>% 
  dplyr::filter(coordinates_obscured == "false", quality_grade != "casual") %>% 
  dplyr::mutate(key = id, scientificName = scientific_name, coordinateUncertaintyInMeters = positional_accuracy, eventDate = observed_on, year = substr(eventDate, 1, 4), source = "iNaturalist", dataset = "iNaturalist", URL = url) %>% 
  dplyr::select(data_fields) %>% 
  dplyr::mutate(key = as.character(key), eventDate = as.character(eventDate), year = as.numeric(year), longitude = as.numeric(longitude), latitude = as.numeric(latitude))
...
idigbio_occurrences_dangermond_cleaned <- idigbio_occurrences_dangermond %>% 
  dplyr::mutate(key = uuid, scientificName = paste0(toupper(substr(name, 1, 1)), substr(name, 2, nchar(name))), coordinateUncertaintyInMeters = coordinateuncertainty, eventDate = datecollected, year = substr(eventDate, 1, 4), source = "iDigBio", dataset = toupper(institutioncode), URL = paste0("https://www.idigbio.org/portal/records/", key)) %>% 
  dplyr::select(data_fields) %>% 
  dplyr::mutate(key = as.character(key), eventDate = as.character(eventDate), year = as.numeric(year), longitude = as.numeric(longitude), latitude = as.numeric(latitude))

# Integrate datasets
integrated_dangermond_occurrences <- list(calflora_occurrences_dangermond_clean,
                               gbif_occurrences_dangermond_clean,
...
dangermond_taxa_info_gnr <- furrr::future_map(dangermond_taxa, purrr::safely(function(x) taxize::gnr_resolve(x, fields = "all")))
## Check data source returning results for most taxa
dangermond_taxa_info_gnr_sources <- purrr::map(dangermond_taxa_info_gnr, function(x) x$result$data_source_title %>% unique()) %>% unlist() %>% table() %>% sort(decreasing = TRUE)
## Extract taxonomic information from GBIF Backbone Taxonomy
dangermond_taxa_info_gbif <- purrr::map(dangermond_taxa_info_gnr, function(x) (x$result %>% dplyr::filter(data_source_title == "GBIF Backbone Taxonomy"))[1, ]) %>% 
  bind_rows() %>% 
  dplyr::filter(complete.cases(matched_name),
                classification_path != "") 
...
# Save output as CSV
occurrences_dangermond %>% readr::write_csv(paste0("data/integrated_occurrences_dangermond.csv"))
